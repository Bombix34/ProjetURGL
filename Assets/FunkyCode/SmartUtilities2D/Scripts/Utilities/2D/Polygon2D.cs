...
public class Polygon2D  {
	...
	static public Polygon2D CreateIsometric(Vector2 size) {
		...
		Polygon2D polygon = new Polygon2D();
		polygon.pointsList.Add(new Vector2D(-size.x, size.y - 0.25f));
		polygon.pointsList.Add(new Vector2D(0, 0 - 0.25f));
		polygon.pointsList.Add(new Vector2D(size.x,  size.y- 0.25f));
		polygon.pointsList.Add(new Vector2D(0, size.y * 2- 0.25f));
		polygon.Normalize();
		return(polygon);
	}
