...
public class ProfileEditor {
	...
	static public void DrawLightLayers(LightPresetLayers presetLayers) {
		...
		for(int i = 0; i < layerSetting.Length; i++) {
			...
			if (foldout) {
				...
				layer.layerID = (LightingLayer)EditorGUILayout.Popup("Layer", (int)layer.layerID, Lighting2D.Profile.layers.lightLayers.GetNames());
	...
	public class QualitySettings {
		...
		public static void Draw(LightingSettings.Profile profile) {
			...
			EditorGUI.indentLevel++;
				...
				profile.qualitySettings.managerInstance = (LightingSettings.QualitySettings.ManagerInstance)EditorGUILayout.EnumPopup("Manager Instance", profile.qualitySettings.managerInstance);

				profile.qualitySettings.managerInternal = (LightingSettings.QualitySettings.ManagerInternal)EditorGUILayout.EnumPopup("Manager Internal", profile.qualitySettings.managerInternal);
	...
	public class FogOfWar {
		// Fog Of War Preset????
		public static void Draw(LightingSettings.Profile profile) {
			bool foldout = GUIFoldoutHeader.Begin( "Fog of War Buffer", profile.fogOfWar); 
