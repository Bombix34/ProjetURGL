...
public class DayLightingCollider2DEditor : Editor {
	...
	override public void OnInspectorGUI() {
		...
		script.mainShape.shadowType = (DayLightingCollider2D.ShadowType)EditorGUILayout.EnumPopup("Shadow Type", script.mainShape.shadowType);


		EditorGUI.BeginDisabledGroup(script.mainShape.shadowType == DayLightingCollider2D.ShadowType.None);
		
		script.shadowLayer = EditorGUILayout.Popup("Shadow Layer (Day)", script.shadowLayer, Lighting2D.Profile.layers.dayLayers.GetNames());
		...
		script.maskLayer = EditorGUILayout.Popup("Mask Layer (Day)", script.maskLayer, Lighting2D.Profile.layers.dayLayers.GetNames());
