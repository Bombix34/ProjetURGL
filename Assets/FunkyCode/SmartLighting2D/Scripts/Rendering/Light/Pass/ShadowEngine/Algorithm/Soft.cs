...
namespace Rendering.Light.Shadow {
    ...
    public class Soft {
        ...
        public static void Draw(List<Polygon2D> polygons) {
			...
			float lightCoreSize = light.coreSize;
			...
			for(int i = 0; i < polygons.Count; i++) {
				...
				int pointsCount = pointsList.Count;
				 ...
				 for(int x = 0; x < pointsCount; x++) {
					...
					float p_a_x = (float)edge_local.A.x;
					float p_a_y = (float)edge_local.A.y;

					float p_b_x = (float)edge_local.B.x;
					float p_b_y = (float)edge_local.B.y;
					...
					//////////////////////////////////////////////
					edge_world_left.x = (float)edge_local.B.x + offset.x;
					edge_world_left.y = (float)edge_local.B.y + offset.y;
					...
					//////////////////////////////////////////////
					edge_world_right.x = (float)edge_local.A.x + offset.x;
					edge_world_right.y = (float)edge_local.A.y + offset.y;
					...
					if (intersection != null) {
						...
						////////// MIDLE
						GL.TexCoord3(penumbraRect.x1, penumbraRect.y0 + penumbraRect.y1  + penumbraSize.y / 2, 0);
						GL.Vertex3(core_left_90_edge.x, core_left_90_edge.y, z);

						GL.TexCoord3(penumbraRect.x0, penumbraRect.y1 + penumbraSize.y / 2, 0);
						GL.Vertex3(mid_x, mid_y, z);

						GL.TexCoord3(penumbraRect.x0, penumbraRect.y0 + penumbraSize.y / 2, 0);				
						...
						if (intersectionRight != null) {
							...
							GL.TexCoord3(penumbraRect.x1, penumbraRect.y0 + penumbraRect.y1, 0);
							...
							// Last Fine Right
							GL.TexCoord3(penumbraRect.x0 + penumbraSize.x / 2, penumbraRect.y1 + penumbraSize.y / 2, 0);
							GL.Vertex3(core_left_90_edge.x, core_left_90_edge.y, z);
							
							GL.TexCoord3(penumbraRect.x0 + penumbraSize.x / 2, penumbraRect.y0 + penumbraSize.y / 2, 0);	
							GL.Vertex3(edge_right_cross2.x, edge_right_cross2.y, z);

							GL.TexCoord3(penumbraRect.x1 + penumbraSize.x / 2, penumbraRect.y0 + penumbraRect.y1  + penumbraSize.y / 2, 0);
