...
namespace DayLighting {
    ...
    public class TilemapShadowMesh {
        ...
        public void GenerateFill(DayLightingTile tile, float height) {
            List<Polygon2D> polys = tile.polygons;
            ...
            foreach(Polygon2D polygon in polys) {
               ...
               // worldPolygon.ToScaleItself(transform.localScale); 
                ...
                int pointsCount = polygonShadowFill.pointsList.Count;

                for(int i = 0; i < pointsCount; i++) {
                    Vector2D a = polygonShadowFill.pointsList[(i - 1 + pointsCount) % pointsCount];
                    Vector2D b = polygonShadowFill.pointsList[i];
                    Vector2D c = polygonShadowFill.pointsList[(i + 1) % pointsCount];
