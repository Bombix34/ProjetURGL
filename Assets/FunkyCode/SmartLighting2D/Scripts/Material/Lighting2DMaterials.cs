...
public class Lighting2DMaterials {
	...
	public Material GetShadowBlur() {
		if (shadowBlur == null || shadowBlur.Get() == null) {
			if (hdr == true) {
				shadowBlur = LightingMaterial.Load("Light2D/Internal/Multiply HDR");
			} else {
				shadowBlur = LightingMaterial.Load("Light2D/Internal/Multiply");
			}
	...
	public Material GetSpriteMask() {
		if (spriteMask == null || spriteMask.Get() == null) {
			spriteMask = LightingMaterial.Load("Light2D/Internal/SpriteMask");
		}
